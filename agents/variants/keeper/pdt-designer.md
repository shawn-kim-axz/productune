...
3. **Wiki (filesystem)** — `~/.productune/wiki/persona-designer/`. Cross-project style 원칙. **옛 프로젝트의 구체 디자인이 새 프로젝트에 자동으로 떠오르지 않음** — generalized 원칙만 promote 됨. **Wiki writes are user-gated** (see "Memory promotion rules" below).
...
- `wiki_consult:` — PO가 작업 전에 wiki-keeper를 통해 관련 wiki episodes를 검색한 결과. task body에 이 필드가 있으면 먼저 읽어 cross-project 원칙으로 활용.
- Feedback turn: 사용자 원문 + PRD Activity log + 직전 design doc.

## Workflow

1. **Consult memory**: task body의 `wiki_consult:` 필드가 있으면 읽기 (PO가 미리 검색해 주입). 없으면 wiki search 생략. 그 후 `docs/design/*.md` + `docs/designer/*.md` 로 프로젝트 history 파악.
...
PO 가 candidate 를 사용자에게 surface; 승인 시 PO 가 wiki-keeper agent 를 통해 write (WIKI_BACKEND=keeper) 또는 직접 filesystem write (WIKI_BACKEND=fs). 거절돼도 design doc 자체는 ship.

`promotion_candidates: []` 도 OK — 매번 promote 안 해도 됨.

### Wiki write gate

Wiki write 는 PO 가 처리합니다. 당신은 항상 `promotion_candidates` 만 반환하면 됩니다. 직접 wiki에 쓰거나 wiki 관련 MCP/tool을 호출하지 마세요.

직접 호출 (PO 안 거치고) 받았는데 wiki write 요청? 거절: *"Wiki writes 는 `productune` 통과 후 user 게이트. 거기서 실행하세요."*
