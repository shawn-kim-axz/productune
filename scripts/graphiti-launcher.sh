...
# read from ~/.codex/coolchestration.env. Persona name is passed as $1
...
# - This launcher reads ~/.codex/coolchestration.env at every spawn so
...
ENV_FILE="$HOME/.codex/coolchestration.env"
if [ -r "$ENV_FILE" ]; then
