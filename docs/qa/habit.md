...
- Cross-link: `bookshelf/fail-patterns.md` (per-version fail log) · `version-summaries/<version>.md` (Phase 5 close) · `bookshelf/cua-vm-harness.md` (real-OS escalation driver).

## Entries

- Real-OS / TCC / OAuth first-run AC (권한 프롬프트 발생여부 · 무결 first-run · 패키징 `.app` 런치 · Automation) → playwright-electron 스모크 사각지대. `type:test`/user-gate일 때만 **cua macOS-VM 하니스**로 검증: `bookshelf/cua-vm-harness.md` (부팅 1클릭은 사람 몫 · 일상은 `soft_reset.sh com.productune.gui` · `--all` 금지 · 하니스 미준비=ENV fail). 평상시 게이트는 그대로 `surfaces.gui` playwright-electron.
