...
**Mockup-as-source**: 본 문서의 layout / panel / pane / tab / chat 스펙은 `docs/artifacts/v0.4/productune/mockups/mockup.html` 을 진실의 출처로 한다 (2026-05-06 결정). 위 6 결정 (단일 PO 세션 / lucide-react / chat-on-right / planner 모드 제거 / git workflow rule / chat.json 단일 파일) 과 충돌 시 결정 우선.
...
| 산출물 | 표현 | 저장 위치 | 승인 기준 |
|---|---|---|---|
| Design system | Rich Markdown + custom React components | `docs/artifacts/<slug>/design-system.md` | 토큰/컴포넌트/상태 표시 규칙이 구현 가능함 |
| Flow diagrams | Mermaid.js | `docs/artifacts/<slug>/flow.md` 또는 `service-flow-and-screens.md` | 전체 stage, 화면 전이, state가 이해됨. Phase 4 GUI 는 Electron/React 내장 viewer 로 inline 렌더 |
| Wireframe | Excalidraw React | `docs/artifacts/<slug>/wireframes/*.excalidraw.json` | workspace, 승인 게이트, 외부 동의 화면이 보임 |
| **Mockup (hi-fi)** | HTML/CSS 정적 프리뷰 | `docs/artifacts/<slug>/mockups/*.html` | non-canonical visual agreement. Build 전 현실감 있는 예시. **본 문서의 source = `productune/mockups/mockup.html`** |
