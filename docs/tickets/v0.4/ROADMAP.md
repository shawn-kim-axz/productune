...
| **T-P4-097** (R4) | Project side panel 2-section split — "현재 버전" + "버전 히스토리" sp-section, current click → `version-current:{id}` 탭 (kanban), past click → `version-history:main` 탭 (linear), tab id in-place rename swap, v1 default 시드 | pdt-developer | dep: T-P4-023 (land), T-P4-095. plan: `docs/artifacts/v0.4/T-P4-097/plan.md`. |
...
| **T-P4-099** (R4) | Sidebar uniform simplify (3 fix) — (A) Version row 2-token + Phase popover 제거 + close badge Phase pill style + "지난 버전" rename / (B) TeamPanel = Personas inline + 제목만 + Skills/Wiki nav row (team-wiki 신규) / (C) Workflow rules → main pane (workflow-settings 신규) | pdt-developer | dep: T-P4-097, T-P4-098, T-P4-024, T-P4-096. plan: `docs/artifacts/v0.4/T-P4-099/plan.md`. L3. |
| **T-P4-102** (R4 fix) | v1 탭 = TicketDashboardView (ticket-review reuse) — T-P4-097 §2(a) revision. SidePanelCurrentVersion onClick = `openTab('ticket-review:'+id, 'ticket-review', {versionFilter:id}, id)`. TicketDashboardView versionFilter prop. VersionHistoryView isCurrentVersion kanban 분기 제거 (~300 line ↓). 신규 TabType 0. | pdt-developer | dep: T-P4-097 (land). plan: `docs/artifacts/v0.4/T-P4-102/plan.md`. L3. |
| **T-P4-103** (R4 fix) | 3 fix 묶음 — (A) Onboarding `onboarding:clearLocalStorage` IPC + preload bridge + wizard step 0 reset CTA (OS-aware macOS/Linux/Windows path). (B) Persona designer role "Plan, Design" en+ko. (C) Persona QA role "Test, Validation" en+ko. | pdt-developer | dep: T-P4-102 + T-P4-022 (land). L2. risk: fs-mutation. |
| **T-P4-104** (R4 doctrine) | Doctrine chunking enforcement — (A) po-instructions.md "Hard rules" 1줄 추가 (designer chunking ≤2/dispatch). (B) PreToolUse hook `pre-chunking-warn.sh` (claude --agent matcher + TASK keyword count → ≥3 시 stderr warn, non-blocking). (C) delegation.md 2 라인 cross-ref 추가. ceiling=2, ROADMAP/Activity 제외. | pdt-developer | plan: `docs/artifacts/v0.4/T-P4-104/plan.md`. L3. |
| **T-P4-105** (R4 fix) | WorkspaceShell board auto-open 제거 — mount useEffect + tickets icon click 둘 다 `current_version` 라우팅. fresh project (no cv) 면 no-op. SidePanelCurrentVersion mount 와 dedup. | pdt-developer | dep: T-P4-102 (land). L1 trivial (~9 line). |
| **T-P4-106** (R4 fix) | productune init 이 user-global doctrine 자동 설치 — `bootstrapUserGlobalDoctrine()` in init.ts. po-instructions.md hash compare + backup, po-memory.md 절대 overwrite X (사용자 누적 손실 방지 AC critical), sections/ + productune.env idempotent seed. --skip-doctrine Escape flag. install.sh 와 idempotent 공존. | pdt-developer | L2. risk: fs-mutation. origin: paepyeong PO doctrine 사고. |
| **T-P4-107** (R4 doctrine) | Persona model/effort defaults + caveman mode 분기 + Designer plan §QA scope 의무 — (a) PO=opus/xhigh, Designer plan/ticket=sonnet/high (L1 exception medium), Dev plan=opus/xhigh, Dev impl=sonnet/high (L1 exception medium), QA=haiku/low (recurring=sonnet/high). (b) sonnet=high / opus=xhigh effort 자동. (c) useUserMode 통합 caveman 분기 (developer 경량 dev어휘 OK / planner 자연어 only / persona간 항상 caveman lite). (d) Designer plan §QA scope 의무 section + PO reject gate. | pdt-developer | plan: `docs/artifacts/v0.4/T-P4-107/plan.md`. 8 file 영향. L3 doctrine. |
| **T-P4-048-mh** (R4) | Settings MCP Servers + Hooks sub-section — SettingsSubTab type 에 'mcp' + 'hooks' 2개 추가 (T-P4-099 nav-only + openTab 패턴 그대로 extend). MCP = ~/.claude/settings.json mcpServers + .mcp.json merge + 편집 modal (Electron main fs.writeFile). Hooks = read-only accordion + settings.json 직접 편집 안내 (toggle = Phase 5 lock). T-P4-022 외부 연결(API token) 과 별 sub-tab 유지. | pdt-developer | plan: `docs/artifacts/v0.4/T-P4-048-mcp-hooks/plan.md`. L3. parent: T-P4-048. (Environment + Models 는 별 dispatch) |
| **T-P4-108** (R4 fix) | T-P4-104 fix-forward — install.sh PreToolUse list 에 pre-chunking-warn.sh entry 추가 + ~/.productune/po-instructions.md §Hard rules chunking 룰 1줄 + sections/delegation.md §PRD delegation 2 라인 cross-ref. install.sh 재실행 시 자동 등록 (end-user impact 0). | pdt-developer | L1. risk: fs-mutation. parent: T-P4-104 partial (3c6cb35). |
| **T-P4-109** (R4 fix) | T-P4-106 fix-forward — productune init shell handler 가 doctrine bootstrap 호출 안 함 (Electron IPC path 만 wire). Option C: packages/core/scripts/lib/bootstrap-doctrine.sh 신규 함수 추출, install.sh + productune init shell 둘 다 source+call. TypeScript src/init.ts = Electron IPC path SoT 명시 분리. --skip-doctrine flag. | pdt-developer | plan: `docs/artifacts/v0.4/T-P4-109/plan.md`. L2. risk: fs-mutation. parent: T-P4-106. origin: paepyeong PO doctrine 사고 root cause. |
...
| **T-P4-052** | Excalidraw React 컴포넌트 embed — 와이어프레임 인터랙티브 편집 | pdt-developer | 저장 = `docs/artifacts/<slug>/<file>.excalidraw.json` |
...
- **2026-05-12 (T-P4-097/T-P4-098 land + T-P4-099 발행 — Sidebar uniform simplify 3 fix)** — T-P4-097 `0c8929e` + T-P4-098 `da319b8` land 후 사용자 dogfood 발견: Version row Phase hover popover 가 "정신없다" + Team panel 8+ inline rows 가 정리 안됨 + Workflow 가 Settings sidebar 안에 inline 렌더 (General 과 비대칭). T-P4-099 = 3 fix 통합: (A) VersionRow 2-token (`[id] · date` / `[id] · close pill`), Phase popover/forceExpanded 제거, "버전 히스토리" → "지난 버전" rename, close badge = Phase pill style reuse (사용자 OQ-A). (B) TeamPanel = Personas section (plain title + inline PersonaRow × 4, collapse toggle 제거 — 사용자 OQ-B) + Skills/Wiki·Memory nav row, team-wiki 신규 tab type, team-personas 미도입. (C) WorkflowRulesPanel → main pane `workflow-settings` 신규 tab type, SettingsView 'workflow' 클릭 시 openTab. 신규 TabType 2: team-wiki + workflow-settings. designer plan opus/high → sonnet/medium revise. dep: T-P4-097/T-P4-098/T-P4-024/T-P4-096. L3.
- **2026-05-13 (T-P4-102 발행 — v1 탭 = TicketDashboardView reuse, T-P4-097 §2(a) revision)** — 사용자 dogfood (image 첨부): `v1` 탭 클릭 시 컨텐츠가 `버전 히스토리 — v1` (VersionHistoryView kanban mode) 으로 표시 — 사용자 mental model 과 mismatch. 사용자 원의도 = TicketDashboardView (ticket-review tab) 그 자체. T-P4-097 plan §2(a) 의 "기존 ticket tab 재사용 = version-history kanban mode" 해석이 잘못. T-P4-102 = (a) SidePanelCurrentVersion onClick `openTab('ticket-review:'+id, 'ticket-review', {versionFilter:id}, id)` (was `version-history`), (b) TicketDashboardView 에 `versionFilter?: string` prop 추가 (undefined → ALL board 동작 보존, set → filter), (c) VersionHistoryView `isCurrentVersion → kanban` 분기 제거 (simplify ~300 line ↓), (d-g) TabType 보존, tab title = versionId verbatim, updateTabId 동일 동작. 신규 TabType 0. plan `docs/artifacts/v0.4/T-P4-102/plan.md` (sonnet/medium, OQ 1-3 default + OQ-4 scope-out). dep: T-P4-097 (land). L3. Designer chunking 정수 (산출물 분리 dispatch — 1차 plan only, 2차 ticket only).
- **2026-05-13 (T-P4-103 발행 — 3 fix 묶음: onboarding LS cleanup + Persona role labels)** — 사용자 dogfood directive: (1) 매번 수동 `rm -rf "$HOME/Library/Application Support/@productune/Local Storage/"` 명령 실행 → onboarding 흐름 자동 cleanup 필요. (2) Team panel Personas section designer role 표시 "UX / 브랜드 / 디자인 시스템" → **"Plan, Design"** (사용자 verbatim). (3) QA role "통과 / 실패 검증" → **"Test, Validation"** (사용자 verbatim). T-P4-103 = (A) `onboarding:clearLocalStorage` IPC handler (electron main.ts) + preload bridge + wizard step 0 reset CTA, OS-aware path (macOS/Linux/Windows), force:true safe deletion. (B) en+ko 라벨 갱신. (C) i18n parity + T-P4-057 protected-token 점검 (Designer/QA name pill 영문 유지). L2, risk fs-mutation. dev dispatch = T-P4-102 + T-P4-022 land 후 순차 worktree. designer ticket emit only (chunking 산출물 1).
- **2026-05-14 (T-P4-091 + T-P4-092 land)** — `99d9751` + `56b7353`. T-P4-091 = App.tsx lastProject existence guard (projectExists IPC + onboarding cleanup, ~46 line). T-P4-092 = T-P4-020 sub-e 완성 (worktree:create + stashAndCreate + commitAndCreate IPC + BaseDirtyModal + ChatPanel trace, 380 keys, OQ 3 stamped b/a/b). A 그룹 backlog cleanup 완료.
- **2026-05-14 (T-P4-104 발행 — doctrine chunking enforcement plan-only)** — 본 conversation 관찰 fix-forward: designer chunking 룰 ("≤2 산출물/dispatch") 3회 위반 cycle (T-P4-099 1차 opus 4산출물 hang → cancel → 2차 sonnet 3 hang → 3차 sonnet 4 결국 분할). plan = `docs/artifacts/v0.4/T-P4-104/plan.md`: (A) `~/.productune/po-instructions.md` "Hard rules" 1줄 추가 (compaction-survive). (B) `pre-chunking-warn.sh` PreToolUse hook (claude --agent matcher + TASK body keyword count + non-blocking warn). (C) delegation.md 2 라인 cross-ref. ceiling=2, ROADMAP/Activity 제외. OQ deferred: --resume 가드 (false positive 위험) / N회 반복 escalate / [ctx].artifacts[] 정확한 카운트. plan only (산출물 1, chunking 정수 준수).
- **2026-05-14 (T-P4-105 발행 — WorkspaceShell board auto-open 제거)** — 사용자 dogfood: "처음에 프로젝트 열면 ticket tab이 여전히 살아있거든. 대신 현재 진행 버젼 탭을 열어줘야돼". 원인: WorkspaceShell mount useEffect L230 + onSelectActivity L225 가 `ticket-review:board` 무조건 open. T-P4-105 = (A) mount: poState.current_version 있으면 `ticket-review:{cv}` + versionFilter, 없으면 no-op. (B) tickets icon click: cv 있으면 `ticket-review:{cv}`, 없으면 board fallback. SidePanelCurrentVersion mount auto-open 과 동일 id → openTab dedup. L1 trivial ~9 line.
- **2026-05-14 (T-P4-106 발행 — productune init 이 user-global doctrine 자동 설치)** — 사용자 directive: "그것도 init에 포함해주면안돼?" (paepyeong PO 의 `~/.productune/po-instructions.md` doctrine 강제 실패 사고 — install.sh 만 user-global doctrine 설치, init 은 project-local 만 처리, 사용자가 init 자주 돌려도 user-global 자동 갱신 X). T-P4-106 = `bootstrapUserGlobalDoctrine()` 신규 함수 in init.ts. po-instructions.md hash compare + .bak.<ts> backup + update (doctrine propagate). po-memory.md 절대 overwrite X (사용자 long-term memory 손실 방지 — AC critical). sections/ + productune.env idempotent seed. --skip-doctrine Escape flag. install.sh 변경 X (idempotent 공존). OQ 3 default 채택 (npm bundle SoT / init 통합 / hash compare). L2, risk fs-mutation.
