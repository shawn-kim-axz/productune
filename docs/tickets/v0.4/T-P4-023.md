...
qa_status: ready
qa_loops: 0
implemented_by: T-P4-023 1차 + 1.5차 + 1.6차 (master-detail UI + kanban/linear 분기 + VERSIONS replace) + T-P4-097 (side panel 2-section split) + T-P4-099 (sidebar uniform simplify A version row) + T-P4-102 (v1 탭 = TicketDashboardView reuse, T-P4-097 §2(a) revision)
notes: 잔여 2차 PR (Filter + Unassigned + Vercel REST + vocab lint) 은 T-P4-022 land + 사용자 dogfood 후 follow-up 으로 결정
---
...
- Design: `docs/designer/service-flow-and-screens.md` §3.2 어휘 매핑 표 + §4 K2 (버전 히스토리 카드)
...
<!-- migrated from docs/artifacts/v0.4/T-P4-023/plan.md -->
...
**Mockups**:
- `docs/artifacts/v0.4/T-P4-023/mockups/main-pane-tab.html` (past-version linear card list — §2.2.2 reference)
- `docs/artifacts/v0.4/T-P4-023/mockups/side-panel-section.html`
...
- **mockup 유효**: 기존 `docs/artifacts/v0.4/T-P4-023/mockups/main-pane-tab.html` spec 그대로 유지 — past version 전용. 신규 mockup 추가 X (kanban 은 이미 ship 된 TicketReviewTab UI 가 reference).
...
### §Plan (additional spec: T-P4-023-version-history-cards)
...
- 한 version 안에 여러 round 가 들어가는 케이스 (예: `v0.4` 안에
...
- **(b) version 기반** — `v0.4` 등 version 별 grouping (T-P4-085
  정합).
...
ticket md (`docs/tickets/v0.4/T-P4-023.md`) 의 acceptance 보강 source. 별
...
3. ticket md (`docs/tickets/v0.4/T-P4-023.md`) acceptance 보강 — 본 plan
