...
| **T-P4-104** (R4 doctrine) | Doctrine chunking enforcement — (A) po-instructions.md "Hard rules" 1줄 추가 (designer chunking ≤2/dispatch). (B) PreToolUse hook `pre-chunking-warn.sh` (claude --agent matcher + TASK keyword count → ≥3 시 stderr warn, non-blocking). (C) delegation.md 2 라인 cross-ref 추가. ceiling=2, ROADMAP/Activity 제외. | pdt-developer | plan: `docs/design/T-P4-104/plan.md`. L3. |
...
| **T-P4-107** (R4 doctrine) | Persona model/effort defaults + caveman mode 분기 + Designer plan §QA scope 의무 — (a) PO=opus/xhigh, Designer plan/ticket=sonnet/high (L1 exception medium), Dev plan=opus/xhigh, Dev impl=sonnet/high (L1 exception medium), QA=haiku/low (recurring=sonnet/high). (b) sonnet=high / opus=xhigh effort 자동. (c) useUserMode 통합 caveman 분기 (developer 경량 dev어휘 OK / planner 자연어 only / persona간 항상 caveman lite). (d) Designer plan §QA scope 의무 section + PO reject gate. | pdt-developer | plan: `docs/design/T-P4-107/plan.md`. 8 file 영향. L3 doctrine. |
| **T-P4-048-mh** (R4) | Settings MCP Servers + Hooks sub-section — SettingsSubTab type 에 'mcp' + 'hooks' 2개 추가 (T-P4-099 nav-only + openTab 패턴 그대로 extend). MCP = ~/.claude/settings.json mcpServers + .mcp.json merge + 편집 modal (Electron main fs.writeFile). Hooks = read-only accordion + settings.json 직접 편집 안내 (toggle = Phase 5 lock). T-P4-022 외부 연결(API token) 과 별 sub-tab 유지. | pdt-developer | plan: `docs/design/T-P4-048-mcp-hooks/plan.md`. L3. parent: T-P4-048. (Environment + Models 는 별 dispatch) |
...
- **2026-05-14 (T-P4-104 발행 — doctrine chunking enforcement plan-only)** — 본 conversation 관찰 fix-forward: designer chunking 룰 ("≤2 산출물/dispatch") 3회 위반 cycle (T-P4-099 1차 opus 4산출물 hang → cancel → 2차 sonnet 3 hang → 3차 sonnet 4 결국 분할). plan = `docs/design/T-P4-104/plan.md`: (A) `~/.productune/po-instructions.md` "Hard rules" 1줄 추가 (compaction-survive). (B) `pre-chunking-warn.sh` PreToolUse hook (claude --agent matcher + TASK body keyword count + non-blocking warn). (C) delegation.md 2 라인 cross-ref. ceiling=2, ROADMAP/Activity 제외. OQ deferred: --resume 가드 (false positive 위험) / N회 반복 escalate / [ctx].artifacts[] 정확한 카운트. plan only (산출물 1, chunking 정수 준수).
