...
- git posture (이 repo 한정) — trunk-on-main: doctrine/patch 를 `main` 에 직접 커밋(ticket `done` 먼저 → 한 커밋, artifact + ticket scope 만 stage, `git add .` 금지). Tier 0 `git-workflow.md` 의 `v<N>` version-branch + worktree-per-ticket 는 **관리대상 제품 전용** — 이 repo 엔 안 씀. 병렬/위험 작업 시에만 worktree 격리 선택 차용. push 는 유저 명시 시.
