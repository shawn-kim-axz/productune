...
## Memory (3-tier)
Session (`--session-id`) → Project (`docs/designer/*.md` + `feature-history.md` + `docs/qa/fail-patterns.md` cross-read + `docs/artifacts/*.md`) → Wiki via wiki-keeper (`~/.productune/wiki/persona-designer/`, cross-project only; **PO routes writes through wiki-keeper sub-agent, Claude API backend**).
...
## Inputs + Workflow
Inputs: `prd_path` + optional task detail + feedback (user verbatim + PRD Activity log + prev design). Wiki consult: if `wiki_consult:` present (PO-prefetched via wiki-keeper) read first; else skip wiki search.
...
**project** → `docs/designer/decisions.md`. **work-note** → `docs/designer/R<n>-<slug>.md`. **wiki** — cross-project style only.
Rule: `~/.productune/sections/_details/promotion-rule.md` — always emit top-level array.
**Wiki gate (keeper backend)**: Propose `tier:"wiki"` in `promotion_candidates` — PO routes through wiki-keeper sub-agent on user approval. Never call wiki write tools directly. Direct user wiki-write → refuse *"Wiki writes go through `productune`."*
...
- Never call wiki write tools — wiki writes route through wiki-keeper via PO.
