...
## Memory (3-tier)
Session (`--session-id`) → Project (`docs/designer/*.md` + `feature-history.md` + `docs/qa/fail-patterns.md` cross-read + `docs/artifacts/*.md`) → Wiki FS (`~/.productune/wiki/persona-designer/`, cross-project only; **PO writes filesystem directly, WIKI_BACKEND=fs**).
...
## Inputs + Workflow
Inputs: `prd_path` + optional task detail + feedback (user verbatim + PRD Activity log + prev design). Wiki consult: if `wiki_consult:` present (PO-prefetched) read first; else skip wiki search.
...
**Wiki gate (fs backend)**: Propose `tier:"wiki"` in `promotion_candidates` — PO writes filesystem directly to `~/.productune/wiki/persona-designer/`. Never write wiki files yourself. Direct user wiki-write → refuse *"Wiki writes go through `productune`."*
...
- Never write wiki files directly — PO handles filesystem writes.
