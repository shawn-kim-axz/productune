...
- ~~`past_tickets[]`~~ — **removed in v2** (ticket md = SoT; T-P4-065). PO + GUI derive ticket lists by fs scan of `docs/tickets/**/*.md`. Revival: `node scripts/po/scan-tickets.mjs <projectDir>` + jq slug filter. **Staleness hygiene (turn-start)**: `jq '.past_tickets = []'` runs every turn — purges any residual stale data. Field MUST NOT be written in v2.
