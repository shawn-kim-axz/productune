...
## UKS correction capture (Step 3 #14c)
...
**Transition method: chat-driven.** User replies directly in chat — no modal banner or Approve/Modify button.
...
3. **PO writes `pending_gate` to `po-state.json`** (mechanical — field deprecated in GUI; retained for legacy compat) — `{from_phase, to_phase, summary, prompt, emitted_at}`.
...
Doctrine = source of truth. CLI = text prompt; GUI banner removed. Existing Gate 1/2/3 = mid-phase checkpoints, not transition gates (don't write `pending_gate`).
