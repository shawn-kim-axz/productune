...
- Update `docs/tickets/<version>/T-NNN.md` lifecycle metadata when routing/closing: status, timestamps, duration, assignee/routing/model/effort/progress refs only. No body/request/acceptance/scope edits.
  `status:` enum enforced by `pre-frontmatter-lint.sh` PreToolUse hook.
...
**Persona output**: JSON-only. `summary` (≤200 char) + optional `user_surface` (≤500 char) carry human content — PO paraphrases for user. Shared doctrine: `sections/_formats/persona-output-format.md`.
...
L4+ goes **plan-first (dev opus/xhigh) → PO reviews (opus/xhigh) → auto-accept impl (sonnet/high)**. PO = default reviewer. L1–L3 trivials skip → straight sonnet/medium impl.
...
2. PO review — testability + acceptance + architecture + risk. opus/xhigh default; opus/max risk. Verdict: `OK` or `revise:[...]`.
...
**Per-ticket fresh / per-turn resume.** Ticket close (status → `done`|`blocked`|`abandoned`) → `jq '.current_task.persona_sessions = {}'` immediately (before `current_task = null`). Next ticket's first dispatch = **no `--session-id`** (fresh). Within-ticket multi-turn (plan→impl, QA retry) = `--resume "$SID"` OK.
