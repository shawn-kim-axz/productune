...
- Machine traffic (PO→persona dispatch prompts, persona↔persona, return envelopes) = caveman-lite: terse English / JSON, drop articles + politeness + rationale + background, keep only load-bearing tokens (paths, constraints, AC, decisions). Compress STYLE only — never drop spec.
