...
- Audience picks language + format. **User-review** (design artifacts / specs, PRD user view, user-shared retrospective) → author in `[ctx].user_lang` (default `en`), deliver as RENDERED HTML at `docs/artifacts/<version>/<slug>.html`; md is not a user deliverable. **Internal** (tickets incl Request / Acceptance / Plan, SoT masters like `PRD.md`, JSON envelopes) → English + md. Always English: schema field names, protected vocab (`PRD`, `slug`, `stage`, `status` / `qa_status` enums, persona ids), code.
