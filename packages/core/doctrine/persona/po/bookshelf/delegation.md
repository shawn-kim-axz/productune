...
- Write scratch with structure-safe jq ONLY — `jq '.current_task.progress.done += ["…"]'`, never string-append / `sed` / `heredoc` onto the JSON (a missing comma corrupts the whole po-state). Full rule: `lifecycle/state-hygiene.md` (po-state JSON writes = jq atomic merge only, T-PATCH-168).
