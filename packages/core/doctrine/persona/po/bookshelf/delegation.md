...
PO orchestrates persona invocations. Never authors persona output. ≤100 lines.
...
Ticket `## Plan` is **Designer-authored** (designer writes plan into ticket body).
PO never edits Plan rows. After delegation: if Designer ships plan changes, PO mirrors
**only** lifecycle frontmatter (status, model, effort, ts). Body diffs surface via
Designer's `summary` field — PO appends 1-line entry to `## Persona Activity` table.

## Plan mode — L5+ plan-only doctrine

L5+ goes **plan-first → PO reviews → impl dispatch**:
...
2. **PO review** — testability + acceptance + architecture + risk. opus / xhigh default.
...
5. **Failure regress** — self-verify / QA fail after Path 1 retry → back to plan
   (opus / xhigh) + PO re-review. `escalation_triggered=true`. Bump `actual_complexity`.
...
**Why explicit**: `claude --print` doesn't auto-engage plan mode; task-body
`PLAN MODE — DO NOT WRITE CODE` is the only non-interactive enforcement.

## Dev-QA auto-loop

After impl dispatch (status → `in-progress`), PO auto-dispatches QA without user confirm.
...
**Per-ticket fresh / per-turn resume.** Ticket close (`done|blocked|abandoned`) →
...
Rationale: 5-ticket session ~$8.6 → per-ticket fresh ~$2.5 (↓70%); crossover ~3 tickets.

## Chunking — per-call size

PO splits multi-area / multi-decision / multi-output directives per sub-area. Per-persona
...
Designer returns `state:"needs-info"` (PO relays `next_question`) or `state:"ready"`
(PRD path + tickets + ambiguity_score + version_outcome). Hard cap 5 needs-info loops;
6th turn = PO ships "finalize" resume body. Full: `prd-clarity-loop.md` (designer bookshelf).
