...
After a persona returns, inspect 4 quality signals. Any one trips → surface a 3-option
menu to the user.
...
Surface all 3 options at once. English template, render in user lang:
...
`max` is NOT reachable via Path 1 — it's a Step 1 routing choice for net-new product
thinking only (PRD R1, design system from scratch, system arch — see `routing.md`).
A persona failing at `xhigh` → don't reach for max; revisit task framing (Path 2 skill
search) or step the user in (Path 3 surface).

## Path 2 — Skill search and apply

Call `skill-fetch search "<query>"`. Build query from `unresolved` items or task keywords:
...
- not installed → emit manual install instructions → on user OK, re-invoke.
- install fail / skill not fit → fall back to Path 1.

## Path 3 — Just proceed

User accepts result as-is. Surface `unresolved` in final summary's "Follow-ups".
...
3-option menu firing → your Step 1 routing was an **under-estimate**:
...
Path 3 (just proceed) is NOT counted as escalation — *unless* `user_rework_requested = true`
next turn. `max` never appears in `escalation=` — not an escalation outcome;
`actual=opus/max` means you chose `max` at Step 1.
